...
(function($) {
  ...
  $.fn.uniform = function(options) {
    ...
    function doSelect(elem){
      ...
      if(selected.text().length > options.maxSelectChars){
        var truncSpan = selected.text().substr(0, options.maxSelectChars);
      ...
      elem.bind({
        "change.uniform": function() {
          ...
          if(spanTag.text().length > options.maxSelectChars){
			...
			if(body.length > options.length + options.minTrail) {
				...
				if(splitLocation != -1) {
					...
					if (obj.children().length > 1) {
					   var first_tag = obj.children().first();
   					var str1 = first_tag.text().substring(0, splitLocation);
					   var str2 = body.substring(splitLocation+(2+(2*first_tag.get(0).tagName.length)+3-4), body.length - 1);
					} else {
